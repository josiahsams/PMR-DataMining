...
class Controller1 @Inject() extends Controller {
   ...
   */
     ...
     val act_cus_name = sqlContext.sql("SELECT DISTINCT ( `Customer Number` ), (`Customer Name`)  from df12  limit 30 ")
   ...
   val num_count = sqlContext.sql("SELECT `Customer Number` , count(1) as cnt from df7 where ( `Critical Situation Description` REGEXP '[0-9]') group by `Customer Number` order by cnt desc limit 30")
